...
import { pageTransitionDetector } from '../../../src/detectors/page-transition.js';
import { pathToFileURL } from 'url';
import { resolve } from 'path';

const fixturesDir = resolve(process.cwd(), 'tests/fixtures');

describe('pageTransitionDetector', () => {
  ...
  it('returns false for static page', async () => {
    ...
    try {
      ...
      await page.goto(url);
      expect(await pageTransitionDetector.detect(page)).toBe(false);
    } finally {
